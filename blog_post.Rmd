...
```{r lp3_notowania_09, message=FALSE, warning=FALSE, error=FALSE}
notowania_analiza %>%
...
```{r lp3_notowania_13, message=FALSE, warning=FALSE, error=FALSE, fig.height=10}
notowania_analiza %>%
  filter(Artist %in% top_artists) %>%
  group_by(Artist, Title) %>%
  summarise(Punkty = sum(Punkty)) %>%
...
```{r lp3_notowania_24, message=FALSE, warning=FALSE, error=FALSE}
notowania_analiza %>%
   group_by(Tag) %>%
   summarise(Punkty=sum(Punkty)) %>%
   ungroup() %>%
